function fish_prompt --description 'Write out the prompt'
	...
	if [ $last_status = 0 ]
		...
		echo -n -s "$__fish_prompt_name" "$USER" "$__fish_prompt_normal" ': ' "$__fish_prompt_cwd" (prompt_pwd) "$__fish_prompt_normal" "$__fish_prompt_virtualenv_msg" "$__fish_prompt_symbol"
